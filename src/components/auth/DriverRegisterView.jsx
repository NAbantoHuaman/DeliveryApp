...
import { User, Lock, FileText, Phone, Mail, Bike, Car, Truck, Upload, CheckCircle2, ChevronRight, ChevronLeft } from 'lucide-react';
...
const DriverRegisterView = ({ onBack, onRegister, isProcessing }) => {
    ...
    const steps = [
        ...
        { id: 2, title: "Tu Vehículo", icon: Bike },
            ...
            <div className="bg-white w-full flex-1 rounded-t-[3rem] shadow-[0_-10px_40px_rgba(0,0,0,0.2)] flex flex-col px-8 pt-8 pb-8 z-30 mt-4 overflow-y-auto">
                <form onSubmit={handleSubmit} className="space-y-5 w-full">
                    ...
                    <div className="mt-auto pt-4">
                        <button 
                            type="submit"
                            disabled={isProcessing || (step === 3 && Object.values(uploadStatus).filter(s => s === 'success').length < (formData.vehicleType === 'bici' ? 2 : 3))}
                            className={`w-full text-white text-xl font-medium py-4 rounded-2xl shadow-lg active:scale-[0.98] transition-all flex items-center justify-center gap-2 ${
                                isProcessing || (step === 3 && Object.values(uploadStatus).filter(s => s === 'success').length < (formData.vehicleType === 'bici' ? 2 : 3))
